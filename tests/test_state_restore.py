    ...
    return _archive(
        {
            ...
            "state/checkpoint.json": {"batch_id": "batch-200", "completed_task_ids": ["a"]},
            "state/history.json": [],
        }
    )
    ...
    poisoned = _archive(
        {
            ...
            "state/checkpoint.json": b"x" * (2 * 1024 * 1024 + 1),
        }
    )
    with pytest.raises(InvalidStateArtifact, match="size"):
        restore_state_archive(
            poisoned,
            destination,
            repo="crawl_vps_promotions",
...
def test_history_is_event_append_deduped_and_retained_for_180_days():
    old = [
        ...
        {"event_id": "same", "observed_at": "2026-07-29T00:00:00Z", "amount": 10},
    ]
    current = [
        {"event_id": "same", "observed_at": "2026-07-29T00:00:00Z", "amount": 10},
        {"event_id": "new", "observed_at": "2026-07-30T00:00:00Z", "amount": 9},
    ]
...
def test_state_writer_emits_complete_atomic_v4_state(tmp_path):
    ...
    write_state_directory(
        ...
        run_id="201",
        config_sha256=CONFIG_SHA,
        batch_id="crawl_vps_promotions:201:1",
        completed_task_ids=["target-one"],
    ...
    root_layout = _archive(
        {
            ...
            "checkpoint.json": {"batch_id": "batch-200", "completed_task_ids": ["a"]},
            "history.json": [],
        }
    )
