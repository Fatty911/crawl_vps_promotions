...
def test_vps_monitor_workflow_runs_deals_before_structural_gate():
    ...
    assert "Fetch review/AFF deal intel" in wf
    assert "--deals --site-dir site" in wf
    deals_pos = wf.find("Fetch review/AFF deal intel")
    gate_pos = wf.find("Structural publish gate")
    upload_state_pos = wf.find("Upload early state")
    assert upload_state_pos < deals_pos < gate_pos
