...
def test_delivery_gate_rejects_generated_and_out_of_scope_paths():
    assert validate_paths(["vps_monitor/monitor.py", "tests/test_monitor.py"]) == []
...
def test_commit_review_trailers_require_two_distinct_pass_families_and_diff_hash():
    valid = {
        ...
        "Reviewed-Diff-SHA256": "b" * 64,
    }
    assert validate_review_trailers(valid, "b" * 64) == []
    invalid = {**valid, "Review-Model-Family-2": "family-a"}
    assert "families" in " ".join(validate_review_trailers(invalid, "b" * 64))
